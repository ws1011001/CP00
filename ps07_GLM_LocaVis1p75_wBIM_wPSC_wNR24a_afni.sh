...
#  rm -r $wdir/confounds/*.1D
  ...
  3dcalc -a $wdir/$oglm/stats.${subj}_${task}+tlrc. -b $kdir/group_${spac}_mask-gm0.2_res-${task}.nii.gz \
    -expr 'a*b' -prefix $wdir/$oglm/stats.gm_${subj}_${task}+tlrc.
  3drefit -addFDR $wdir/$oglm/stats.gm_${subj}_${task}+tlrc.
done
## ---------------------------
