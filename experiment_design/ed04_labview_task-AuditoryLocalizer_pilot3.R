...
setwd(pdir)
odir <- file.path(pdir,'Localizers')
# scan parameters
...
if (aloc.create){
  ...
  aloc.cond.blks01 <- ''
  aloc.cond.blks32 <- ''
  while ( (aloc.cond.rep!=cond.blks.num) | (aloc.cond.blks01!='Words') | (aloc.cond.blks32!='Rest') ){
    ...
    aloc.cond.blks01 <- aloc.cond.blks[1]
    aloc.cond.blks32 <- aloc.cond.blks[32]
  }
...
## output
# save R data
save(list=ls(),file=file.path(odir,'Stimulation_task-AuditoryLocalizer_Pilot3.Rdata'))
# output timings
sink(file=file.path(odir,'Stimulation_task-AuditoryLocalizer_Pilot3.stat'),append=FALSE)
...
sink()
# output LabView .desc
write.table(aloc.desc,file=file.path(odir,'Stimulation_task-AuditoryLocalizer_Pilot3.desc'),
            sep='\t',row.names=FALSE,quote=FALSE)  # UTF-8
write.table(aloc.desc,file=file.path(odir,'Stimulation_task-AuditoryLocalizerFr_Pilot3.desc'),
