...
# working path
wdir <- '/home/wang/Projects/SWAP/LabView_Scripts/'
...
badstim <- read.table(file=file.path(pdir,'bad_words.txt'),stringsAsFactors=FALSE)$V1
## ---------------------------
...
pseudowords.pool <- pseudowords.pool[!pseudowords.pool$sfname %in% badstim,]
## ---------------------------
...
words.cmr <- words.cmr[!words.cmr$sfname %in% badstim,]
...
while (any(ComparisonP.1word < 0.1)){
  ...
  units.1word <- units.assignment[1:5]
...
# prepare words
units.2word <- units.assignment[6:16]
