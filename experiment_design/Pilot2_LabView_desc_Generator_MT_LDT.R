...
# function - convert ITI from Pilot1 to Pilot2
iti_p1p2 <- function (x){ ceiling(20*((5.6*2-x)/5.6))-3 }
## ---------------------------
...
main.fseq.op <- main.fseq.op[1:(dim(main.fseq.op)[1]*3/4),]  # reduce 25% trials based on Pilot1
...
main.fseq.op$catch <- as.vector(rbind(rep('none',trials.num),c(sample(catch.run),sample(catch.run),sample(catch.run))))
...
main.trials <- read.csv(file=file.path(pdir,'Pilot2_words_2wordTrials_byTrials.csv'))
...
#    triggers.iti <- ceiling(main.dur.iti/trigger.duration)
    ...
    #main.catchAA.tmp <- c(wav.a1,wav.a2)
...
# split the main task into 3 runs, and make the duration as a multiple of TRs
...
for (irun in 1:runs.num){
  ...
  write.table(main.desc.irun,file=file.path(pdir,sprintf('Stimulation_task-AudioVisAsso2word_Pilot2_Run%d.desc',irun)),
              sep='\t',row.names=FALSE,quote=FALSE)  # UTF-8
  write.table(main.desc.irun,file=file.path(pdir,sprintf('Stimulation_task-AudioVisAsso2wordFr_Pilot2_Run%d.desc',irun)),
              sep='\t',row.names=FALSE,quote=FALSE,fileEncoding='latin1')  # ISO-8859-1 
  sink(file=file.path(pdir,sprintf('Stimulation_task-AudioVisAsso2word_Pilot2_Run%d.stat',irun)),append=FALSE)
...
# save R data
save(list=ls(),file=file.path(pdir,'Stimulation_task-AudioVisAsso2word_Pilot2.Rdata'))
