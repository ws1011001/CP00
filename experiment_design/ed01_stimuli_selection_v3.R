## ---------------------------
## [script name] ed01_stimuli_selection_v3.R
##
## SCRIPT to select words as stimuli for the cross-modal repetition task in the fMRI study. [3rd version].
##
## By Shuai Wang, [date] 2019-11-07
##
## ---------------------------
## Notes:
...
##                       no homophone words
##        Target variables: variables to be matched across eight conditions 
##                          number of letters, number of phonemes, 50% monosyllable and 50% bisyllable, 
##                          word frequency (freqlivres and freqfilms2),
##                          OLD20, PLD20 and Uniqueness Point for spoken words (puphon).
##                          - Distance measures (i.e. OLD20 and PLD20) are more precise than the number of ortho. or
##                            phono. neighbours.
...
##                          - Variables to be controled for each trial: orthographic/phonological distance between 
##                            adjacent words.
## ---------------------------
...
# setup path
wdir <- '/media/wang/BON/Projects/CP00/experiment_design/stimuli_selection/'
setwd(wdir)
#rdir <- paste0('Words_Selection_',format(Sys.time(),"%Y_%m%d_%H%M"))  # results directory
#dir.create(path=rdir)
rdir <- 'Words_Selection_2019_1107_1915'
## ---------------------------

## read and clean the words dataset
if (file.exists('Lexique_Words_v4.Rdata')){
  load('Lexique_Words_v4.Rdata')
} else {
