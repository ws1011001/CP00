...
fmnilocv <- file.path(rdir, 'activation', 'task-LocaVis1p75', 'group_space-MNI152NLin2009cAsym_mask-ilvOT-coordinates.csv')
...
fplot <- file.path(rdir, 'activation', 'group_task-LocaVis1p75_individual-peaks.png')
png(filename = fplot, width = 8, height = 6, unit = 'in', res = 300)
  scatter3D(df_mni_locv$x, df_mni_locv$y, df_mni_locv$z, colvar = df_mni_locv$T, pch = 20, cex = 3, main = 'Individual Peak Coordinates (N = 22)', bty = 'g',
            xlab = 'X', ylab = 'Y', zlab = 'Z', ticktype = 'detailed', clab = 'T value', clim = c(2, 7), col = ramp.col (col = c('yellow', 'red')))
dev.off()
# compare PSCs for ROIs
## ---------------------------
...
# plot comparisons
p_signs <- data.frame(sleft = c(1, 2), sright = c(3, 3), slabs = c('**', '***'), spos = c(0.65, 0.57),
...
p <- rplot_box2(df_psc_loca_thisroi$PSC, df_psc_loca_thisroi$condition, bColor = c('gray40', 'gray60', 'gray80'), fColor = c('gray40', 'gray60', 'gray80'), 
                gOrder = c('words', 'pseudowords', 'scrambled'), gLabel = c('Words', 'Pseudowords', 'Scrambled'), textSz = 15, title = thisroi, 
                aLabel = c('', 'Percent of Signal Change'), Yrange = c(-0.3, 0.7), Xangle = 90, sBars = p_signs)
...
for (iroi in rois){
  ...
  if (sum(p_P$sig) > 0){
    ...
    p_P$lab[p_P$val < 0.001] <- '***'
    p_signs <- data.frame(sleft = c(1:4)[p_P$sig], sright = c(1:4)[p_P$sig], slabs = p_P$lab[p_P$sig], 
                          spos = rep(p_Y[2] - 0.08, sum(p_P$sig)), labsize = 5, vjust = 1, stringsAsFactors = FALSE)   
  } else {
    p_signs <- NULL
  }
  # boxplot
  p <- rplot_box2(df_acc_iroi$ACC, df_acc_iroi$modality, title = iroi, bColor = modalities_color, fColor = modalities_color, 
                  gOrder = modalities, gLabel = modalities, aLabel = c('', 'Accuracy'), Xangle = 90, Yrange = p_Y, sBars = p_signs)+
       geom_hline(yintercept = 0.5, linetype = 'dashed')
  # output figure
  fplot <- file.path(rdir, 'multivariate', sprintf('group_task-AudioVisAssos1word_MVPA-ST3-ACC_ROI-%s_v1.0.png', iroi))
  save_plot(filename = fplot, p, base_height = 5, base_asp = 0.6)
}
## ---------------------------
...
# statistical tests
rse_psc_test <- sapply(unique(df_psc_rses$ROI_label), function(x) 
                       rcomparison_sy(df_psc_rses[df_psc_rses$ROI_label == x,], 'PSC', 'condition', conditions, 'participant_id'),
                       simplify = FALSE, USE.NAMES = TRUE)
rse_fir_test <- list()
for (ifir in FIRs){
  rse_fir_test[ifir][[1]] <- sapply(unique(df_psc_rses$ROI_label), function(x) 
                                    rcomparison_sy(df_psc_rses[df_psc_rses$ROI_label == x,], ifir, 'condition', conditions, 'participant_id', direc = 'less'),
                                    simplify = FALSE, USE.NAMES = TRUE)
}  

# PLOT PSC
for (iroi in unique(df_psc_rses$ROI_label)){
  # extract data for this ROI
  ...
  # boxplot
  p_rse_iroi <- rplot_box2(df_psc_rses_iroi$PSC, df_psc_rses_iroi$condition, title = iroi,
